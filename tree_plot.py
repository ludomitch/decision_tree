...
def add_rectangle(x,y, text, ax):
    ...
    ax.annotate(text, (cx, cy), color=clr, weight='bold', 
                fontsize=rectangle.get_width()/2, ha='center', va='center')
...
def connect_rectangles(Rectangle_1, Rectangle_2, LeftorRight, ax):
    ...
    coordsA = "data"
    coordsB = "data"
    con = mpatch.ConnectionPatch(xyA=(x1,y1), xyB=(x2,y2), coordsA = coordsA, coordsB=coordsB,
                      arrowstyle="-")
    ax.add_artist(con)
    ax.annotate(text, ((x1+x2)/2 + p , (y1+y2)/2 ), color='b', weight='bold', 
                fontsize=10, ha='center', va='center')

def find_input(branch, l_r):
    if ('left' in branch)| ('right'in branch):
        res = {'sig': int(branch[l_r]['info_label']),
        'dB': int(branch[l_r]['dB'])}
    else:
        res = {'label': int(branch[l_r])}
    return res

def make_str(typ, words):
    if typ=='leaf':
        res = "room {label}".format(**words)
    else:
        res = 'signal {sig}>{dB} dB'
    return res

# def rectangle_wrapper():
...
def recurs_plot(x,y, branch, alpha, beta, ax, srec = None):
    ...
    for side in ['left', 'right']:
        ...
        if isinstance(branch[side],float):
            ...
            recurs_plot(
                rec_x2,
                rec_y, branch[side], alpha , beta, ax, srec1)
